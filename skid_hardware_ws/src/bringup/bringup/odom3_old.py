...
class EncoderIMUToOdomNode(Node):
    def __init__(self):
        super().__init__('odom_node_fixed')
        ...
        ## THAY ĐỔI: Thêm tham số tần số để dễ dàng cấu hình
        ...
        self.get_logger().info(f"Odom node started with update frequency: {frequency} Hz")

    ## THAY ĐỔI: Xóa hàm is_valid_rpm không được sử dụng
    ...
    def left_wheel_callback(self, msg):
        ...
        rpm1, rpm2 = msg.data[0], msg.data[1]
        if math.isnan(rpm1) or math.isnan(rpm2):
            # self.get_logger().warn(f"Left wheel RPM contains NaN: {rpm1}, {rpm2}")
            return
        if abs(rpm1) > 500 or abs(rpm2) > 500: # Giới hạn an toàn
            # self.get_logger().warn(f"Left wheel RPM too large: {rpm1}, {rpm2}")
            return
    ...
    def right_wheel_callback(self, msg):
        ...
        rpm1, rpm2 = msg.data[0], msg.data[1]
        if math.isnan(rpm1) or math.isnan(rpm2):
            # self.get_logger().warn(f"Ignoring invalid right wheel RPMs: {rpm1}, {rpm2}")
            return
        if abs(rpm1) > 500 or abs(rpm2) > 500: # Giới hạn an toàn
    ...
    def imu_callback(self, msg):
        # Lấy vận tốc góc (yaw rate) từ IMU. Hàm này không đổi.
    ...
    def update_odom(self):
        ...
        # Tạo và publish message Odometry
        odom = Odometry()
